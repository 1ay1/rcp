---
title: Quickstart
description: Stand up an RCP engine and query it — in Python, Node.js, Rust, or C++ — in under five minutes.
icon: rocket
---

RCP ships **four native SDKs** that speak the identical wire format: a header-only
C++ SDK and dependency-free Python, Node.js, and Rust SDKs. Any client can drive
any server — cross-language interop is proven by the test suites.
